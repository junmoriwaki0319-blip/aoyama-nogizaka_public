...
async function doResetPassword(){var e=document.getElementById("resetEmail").value.trim(),m=document.getElementById("authResetMsg");if(!e){m.textContent="メールアドレスを入力してください";m.className="auth-msg error";return}try{await window.firebaseResetPassword(e);m.textContent="リセット用メールを送信しました。メールをご確認ください。";m.className="auth-msg success"}catch(err){var msg="エラーが発生しました";if(err.code==="auth/user-not-found")msg="このメールアドレスは登録されていません";else if(err.code==="auth/invalid-email")msg="メールアドレスの形式が正しくありません";else if(err.code==="auth/too-many-requests")msg="送信回数が多すぎます。しばらくしてからお試しください";m.textContent=msg;m.className="auth-msg error"}}
