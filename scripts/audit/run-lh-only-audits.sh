#!/bin/bash
set -u
# T2: Lighthouse only-audits runner for 13 URLs (tap-targets, color-contrast, viewport, meta-viewport, font-size)

OUT_DIR="./audits/20260420"
mkdir -p "$OUT_DIR"
LOG="$OUT_DIR/lh-only-audits.log"
: > "$LOG"

ONLY="tap-targets,color-contrast,viewport,meta-viewport,font-size"
...
for entry in "${urls[@]}"; do
  slug="${entry%%|*}"
  url="${entry#*|}"
  out="$OUT_DIR/lh-mobile-only-$slug.json"
  ...
  if [ -f "$out" ]; then
    ...
    echo "[$(date +%H:%M:%S)] FAIL $slug (rc=$rc, no output)" | tee -a "$LOG"
  fi
done
echo "[$(date +%H:%M:%S)] DONE T2 only-audits" | tee -a "$LOG"
