...
def load_edinet_code_map():
    """EDINET コードリストをダウンロードし、edinetCode → {name, sec_code} のマッピングを構築"""
    if not API_KEY:
        return {}
                    ...
                    for row in reader:
                        if header is None:
                            ...
                            if name_col is None:
                                name_col = 6
                            continue
                        max_col = max(c for c in [edinet_col, name_col, sec_code_col] if c is not None)
                        if len(row) <= max_col:
                            continue
                        edinet_code = row[edinet_col].strip()
                        company_name = row[name_col].strip()
                        sec_code = row[sec_code_col].strip() if sec_code_col is not None and sec_code_col < len(row) else ""
                        if edinet_code and company_name:
                            code_map[edinet_code] = {
                                "name": company_name,
                                "sec_code": extract_sec_code(sec_code),
                            }
...
def build_report_entry(doc, activists, xbrl_data=None, edinet_code_map=None):
    ...
    # 2. EDINETコードリストから企業名・証券コード
    if edinet_code_map:
        issuer_code = (doc.get("issuerEdinetCode") or "").strip()
        subject_code = (doc.get("subjectEdinetCode") or "").strip()
        issuer_info = edinet_code_map.get(issuer_code) or edinet_code_map.get(subject_code)
        if issuer_info:
            if not target_name:
                target_name = issuer_info.get("name", "") if isinstance(issuer_info, dict) else issuer_info
            if not sec_code:
                if isinstance(issuer_info, dict) and issuer_info.get("sec_code"):
                    sec_code = issuer_info["sec_code"]
...
def main():
    ...
    # EDINETコードリストで既存データの企業名・証券コードを補完（API不要）
    if edinet_code_map:
        patched = 0
        for r in existing_reports + new_reports:
            issuer_code = (r.get("issuer_name") or "").strip()
            if issuer_code and issuer_code.startswith("E"):
                info = edinet_code_map.get(issuer_code)
                if info and isinstance(info, dict):
                    if not r.get("target_company") and info.get("name"):
                        r["target_company"] = info["name"]
                        patched += 1
                    if not r.get("sec_code") and info.get("sec_code"):
